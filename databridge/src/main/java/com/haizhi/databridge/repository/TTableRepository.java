...
public interface TTableRepository extends HaizhiBaseRepository<TTableBean, String> {
	@Query(value = "select * from t_table where  t_table.db_id = ?1 "
			+ "and t_table.owner = ?2 and t_table.deleted=0", nativeQuery = true)
	Optional<List<TTableBean>> findAllByDbIdAndOwner(String dbId, String owner);

	@Query(value = "select * from t_table where  t_table.tb_name = ?1  and t_table.db_id = ?2"
			+ "and t_table.owner = ?3 and t_table.deleted=0", nativeQuery = true)
	Optional<TTableBean> findByTbNameAndDbIdAndOwner(String tbName, String dbId, String owner);

	@Query(value = "select * from t_table where  t_table.table_id = ?1 "
			+ "and t_table.owner = ?2 and t_table.deleted=0", nativeQuery = true)
	Optional<TTableBean> findByTableIdAndOwner(String tableId, String owner);

	@Query(value = "select * from t_table where  t_table.scheduler_id = ?1 "
			+ "and t_table.owner = ?2 and t_table.deleted=0", nativeQuery = true)
	Optional<List<TTableBean>> findBySchedulerIdAndOwner(String schedulerId, String owner);

	@Query(value = "update t_table set deleted = 1 where table_id = ?1 and deleted=0", nativeQuery = true)
	void logicDeleteByTableId(String tableId);

	@Query(value = "update t_table set deleted = 1 where scheduler_id = ?1 and deleted=0", nativeQuery = true)
